...
contract Choreography_1661x4r is IChannelResolver {
  ...
  function instance(uint _nonce, address[2] memory _participants) external returns (bytes32) {
    ...
    console.log("Choreography_1661x4r: new instance registered with ID (see below)"); console.logBytes32(id);
    return id;
  }
   ...
   */
   function submit(bytes32 id, Step calldata _step) external {
    ...
    if (0 == _step.newState.index && 0 == _disputeMadeAtUNIX) {
      ...
      console.log("Choreography_1661x4r: new dispute (stuck in start event) registered with ID (see below)"); console.logBytes32(id);
    }
    else {
      if (checkStep(id, _step)) {
        ...
        console.log("Choreography_1661x4r: new dispute registered with ID (see below)"); console.logBytes32(id);
      }
    }
  }
