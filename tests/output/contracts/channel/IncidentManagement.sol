   ...
   */
   function submit(bytes32 id, Step calldata _step) external {
    ...
    if (0 == _step.newState.index && 0 == _disputeMadeAtUNIX) {
      ...
      console.log("IncidentManagement: new dispute (stuck in start event) registered with ID (see below)"); console.logBytes32(id);
    }
    else {
      if (checkStep(id, _step)) {
        ...
        console.log("IncidentManagement: new dispute registered with ID (see below)"); console.logBytes32(id);
      }
    }
  }
