...
contract XORAND is IChannelResolver {
  ...
  function instance(uint _nonce, address[3] memory _participants) external returns (bytes32) {
    ...
    console.log("XORAND: new instance registered with ID (see below)"); console.logBytes32(id);
    return id;
  }
   ...
   */
   function submit(bytes32 id, Step calldata _step) external {
    ...
    if (0 == _step.newState.index && 0 == _disputeMadeAtUNIX) {
      ...
      console.log("XORAND: new dispute (stuck in start event) registered with ID (see below)"); console.logBytes32(id);
    }
    else {
      if (checkStep(id, _step)) {
        ...
        console.log("XORAND: new dispute registered with ID (see below)"); console.logBytes32(id);
      }
    }
  }
  ...
  function enact(bytes32 instanceID, uint id) external {
    ...
    while(_tokenState != 0) {
      ...
      if (_tokenState & 6 == 6) {
        if (instanceData[instanceID].state.items == true) {
          // <---  auto transition  --->
          _tokenState &= ~uint(6);
          _tokenState |= 32;
          continue;
