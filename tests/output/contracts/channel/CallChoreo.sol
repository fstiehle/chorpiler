...
contract CallChoreo is IChannelResolver {
  ...
  function instance(uint _nonce, address[3] memory _participants) external returns (bytes32) {
    ...
    console.log("CallChoreo: new instance registered with ID (see below)"); console.logBytes32(id);
    return id;
  }
   ...
   */
   function submit(bytes32 id, Step calldata _step) external {
    ...
    if (0 == _step.newState.index && 0 == _disputeMadeAtUNIX) {
      ...
      console.log("CallChoreo: new dispute (stuck in start event) registered with ID (see below)"); console.logBytes32(id);
    }
    else {
      if (checkStep(id, _step)) {
        ...
        console.log("CallChoreo: new dispute registered with ID (see below)"); console.logBytes32(id);
      }
    }
  }
  ...
  function ChoreographyTask_0hy9n0g(bytes32 instanceID, uint _order) external {
